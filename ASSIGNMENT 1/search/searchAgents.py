...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    unexplored_corners = state[1]
    current_state = state[0]

    # return 0 at every goal state
    if current_state in unexplored_corners:
        return 0
    elif len(unexplored_corners) == 0:
        return 0
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        distanceToClosest = 9999
        # find closest dot using mazeDistance
        for i in food:
            newDistance = mazeDistance(startPosition, i, gameState)
            if newDistance < distanceToClosest:
                distanceToClosest = newDistance
                closest = i

        foodProblem = PositionSearchProblem(gameState, goal=closest, start=startPosition)

        return search.bfs(foodProblem)
